...
export default {
  ...
  mounted() {
    window.ymaps.ready(() => {
      ...
      const myPolygon = this.createMKADArea(myMap);
      let myRoute = null;
      let line = null
      myMap.events.add('click', async (e) => {
        ...
        let infoText = await new Promise((resolve) => {
          window.ymaps.route([coords, closePoint.position], {routingMode: 'auto'})
              .then(route => {
                myRoute = route;
                myMap.geoObjects.add(myRoute)
                myMap.geoObjects.remove(myPlacemark);
                resolve(`Расстояние по дороге: ${this.getDistance(myRoute.getLength())} км. Расстояние по небу: ${this.getDistance(closePoint.distance)} км.`)
              })
        })
        infoText += await new Promise(resolve => {
